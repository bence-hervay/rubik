...
use crate::algorithms::edges::three_cycle::{
...
fn slice_outer_edge_three_cycle_candidate_moves(
    ...
    let slice_half = face_layer_move(
        side_length,
        slice_face,
        slice_depth_from_face,
        MoveAngle::Double,
    );
    let slice = face_layer_move(side_length, slice_face, slice_depth_from_face, slice_angle);
    let outer = face_layer_move(side_length, outer_face, 0, MoveAngle::Positive);
    let outer_half = face_layer_move(side_length, outer_face, 0, MoveAngle::Double);
    ...
    if side_length == 3 {
        for slice_face in FaceId::ALL {
            for outer_face in FaceId::ALL {
                if outer_face == slice_face || outer_face == opposite_face(slice_face) {
                    continue;
                }
...
fn optimized_face_commutators_match_expanded_moves_for(destination: FaceId, helper: FaceId) {
    ...
    assert_ne!(helper, opposite_face(destination));
...
#[test]
fn net_can_render_with_bold_colored_ascii_facelets() {
    let cube = Cube::<Byte>::new_solved(2);
    let net = cube.net_string_with_options(NetRenderOptions::styled_ascii());

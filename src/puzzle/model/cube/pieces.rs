use crate::{geometry, line::StripSpec};
...
pub(super) struct FacePosition {
    pub(super) face: FaceId,
    pub(super) row: usize,
    pub(super) col: usize,
}

pub(super) fn facelet_location(position: FacePosition) -> FaceletLocation {
    FaceletLocation {
        face: position.face,
        row: position.row,
        col: position.col,
...
pub(super) fn trace_position_through_move(
    n: usize,
...
pub(super) fn unique_edge_cubies(
    n: usize,
...
pub(super) fn edge_cubie_location(
    n: usize,
    location: FaceletLocation,
) -> Option<EdgeCubieLocation> {
    if n < 3 || location.row >= n || location.col >= n {
...
pub(super) fn facelet_locations_are_unique(
    locations: impl IntoIterator<Item = FaceletLocation>,
) -> bool {
    let mut seen = [None; 6];
    let mut len = 0;
...
pub(super) fn edge_cubie_sets_match(

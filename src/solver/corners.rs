...
impl CornerMoveSpec {
    ...
    fn move_for_side_length(self, side_length: usize) -> Move {
...
impl CornerMoveTables {
    ...
    fn solve(&self, state: CornerState) -> Option<Vec<CornerMoveSpec>> {
        if state.is_solved() {
            return Some(Vec::new());
        }
...
fn read_corner_state<S: FaceletArray>(cube: &Cube<S>) -> Option<CornerState> {
    if cube.side_len() < 2 {
...
fn all_corner_facelets_solved<S: FaceletArray>(cube: &Cube<S>) -> bool {
    let side_length = cube.side_len();
    if side_length < 2 {
        return true;
    }
